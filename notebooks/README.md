...
| [lab-09-multi-source-routing.ipynb](lab-09-multi-source-routing.ipynb) | Lab 09 | `genai_enrichment` (+ extra source) | A second knowledge source on a new topic; per-question routing across multiple indexes, with grounded answers. |
| [lab-10-comparison-summary.ipynb](lab-10-comparison-summary.ipynb) | — | `visual_nlp` | A rubric-scored 2-question × 4-mode matrix, full answers side by side, and every lab's recorded run, with a mode-selection guide. |
...
The comparison notebook (lab-10) scores each answer against a **rubric** — the key
...
`lab-10-comparison-summary.ipynb` if you want its "recorded runs" table populated
